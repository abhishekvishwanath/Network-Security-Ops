...
class DataIngestion:
    ...
    def split_data_as_train_test(self, df: pd.DataFrame) -> None:
        try:
            ...
            # train_test_split returns tuple of DataFrames, type checker may not infer correctly
            train_set, test_set = train_test_split(df, test_size=self.data_ingestion_config.test_size, random_state=42)  # type: ignore
            ...
            train_set.to_csv(self.data_ingestion_config.train_file_path, index=False, header=True)  # type: ignore
            test_set.to_csv(self.data_ingestion_config.test_file_path, index=False, header=True)  # type: ignore
        except Exception as e:
            raise CustomException(e, sys)
